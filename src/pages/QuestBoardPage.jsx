...
export function QuestBoardPage() {
  ...
  // Fetch quests from Firestore on load
  const fetchQuests = async () => {
    try {
      ...
      console.error("Error fetching quests from database:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Post a new quest to Firestore
  const handlePostQuest = async (e) => {
    e.preventDefault();
    
    // Fallback if user is not logged in yet
    ...
    try {
      ...
      console.error("Failed to post quest to Firestore:", error);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-stone-800 border border-amber-800 rounded-md text-amber-100 placeholder-amber-900 focus:outline-none focus:ring-2 focus:ring-amber-600"
          />
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-md transition-colors flex items-center gap-2 cursor-pointer ${
              filter === "all" ? "bg-amber-600 text-white" : "bg-stone-800 text-amber-200 border border-amber-800 hover:bg-stone-700"
            }`}
          >
            <Filter className="w-4 h-4" /> All
          </button>
          <button
            onClick={() => setFilter("looking-for-players")}
            className={`px-4 py-2 rounded-md transition-colors flex items-center gap-2 cursor-pointer ${
              filter === "looking-for-players" ? "bg-amber-600 text-white" : "bg-stone-800 text-amber-200 border border-emerald-700"
            }`}
          >
            <Users className="w-4 h-4" /> LFP
          </button>
          <button
            onClick={() => setFilter("looking-for-group")}
            className={`px-4 py-2 rounded-md transition-colors flex items-center gap-2 cursor-pointer ${
              filter === "looking-for-group" ? "bg-amber-600 text-white" : "bg-stone-800 text-amber-200 border border-blue-700"
            }`}
          >
            <User className="w-4 h-4" /> LFG
          </button>
        </div>

        <button
          onClick={() => setShowNewQuestModal(true)}
          className="px-6 py-3 bg-amber-600 hover:bg-amber-700 text-white rounded-md transition-colors flex items-center gap-2 cursor-pointer"
        >
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="text-center py-12 text-amber-500 animate-pulse font-medium">
            Consulting the tavern logs...
          </div>
          ...
          <div className="bg-stone-800/40 border-2 border-dashed border-amber-800/50 rounded-lg p-12 text-center">
            ...
            <p className="text-amber-400/60 mb-6">Be the first to post a quest and gather your party.</p>
            <button
              onClick={() => setShowNewQuestModal(true)}
              className="px-6 py-2 bg-stone-700 hover:bg-stone-600 text-amber-100 rounded-md transition-colors inline-flex items-center gap-2 cursor-pointer"
            >
              <Plus className="w-4 h-4" /> Create the first Quest
            </button>
          </div>
        ) : (
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                        quest.type === "looking-for-players" ? "bg-emerald-900/50 text-emerald-300 border border-emerald-700" : "bg-blue-900/50 text-blue-300 border border-blue-700"
                      }`}
                    >
                  ...
                  {quest.tags && quest.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-3">
                      {quest.tags.map((tag, index) => (
                        <span key={index} className="px-2 py-1 bg-amber-900/30 border border-amber-800/50 rounded text-xs text-amber-300">
                          {tag}
                        </span>
                      ))}
                    </div>
            ...
            <form onSubmit={handlePostQuest} className="space-y-4">
              <div>
                ...
                <select 
                  ...
                  className="w-full px-4 py-2 bg-stone-900 border border-amber-800 rounded-md text-amber-100 focus:outline-none"
                >
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-2 bg-stone-900 border border-amber-800 rounded-md text-amber-100 placeholder-amber-900 focus:outline-none" 
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full px-4 py-2 bg-stone-900 border border-amber-800 rounded-md text-amber-100 placeholder-amber-900 resize-none focus:outline-none" 
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-2 bg-stone-900 border border-amber-800 rounded-md text-amber-100 placeholder-amber-900 focus:outline-none" 
                  placeholder="e.g., D&D 5e, Online, Beginner Friendly" 
                />
              </div>
              
              <div className="flex gap-3 pt-4">
                <button type="submit" className="flex-1 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-md transition-colors font-semibold cursor-pointer">
                  Post Quest
                </button>
                <button 
                  type="button" 
                  onClick={() => setShowNewQuestModal(false)} 
                  className="flex-1 py-2 bg-stone-700 hover:bg-stone-600 text-amber-200 rounded-md transition-colors cursor-pointer"
                >
                  Cancel
                </button>
              </div>
